...
mod tests {
    ...
    #[test]
    fn test_author_deserialize_and_user_folder() {
        let json_data = json!({
            "role": "tool",
            "name": "t2uay3k.sj1i4kz",
            "metadata": {
                "image_gen_task_id": "chatimagegen-us-prod.fck9d:user-SEUtqkBUVU4hhsZMKB1C77P5-abcdef:US"
            }
        });

        // Deserialize
        let author: Author = serde_json::from_value(json_data).unwrap();

        // Check basic fields
        assert_eq!(author.role, "tool");
        assert_eq!(author.name.as_deref(), Some("t2uay3k.sj1i4kz"));

        // Check user_folder extraction
        let folder = author.user_folder().unwrap();
        assert_eq!(folder, "user-SEUtqkBUVU4hhsZMKB1C77P5");
    }

    #[test]
    fn test_author_without_image_gen_task_id() {
        let json_data = json!({
            "role": "user",
            "name": "Alice",
            "metadata": {}
        ...
        let author: Author = serde_json::from_value(json_data).unwrap();
        assert_eq!(author.user_folder(), None);
    }
}